...
/* =====================================================================
   Software 3D fallback (no WebGL 2): shaded surface points with a z-buffer
   ===================================================================== */
...
function extractSoftPoints() {
  ...
  const key = V.rgba.length + '|' + V.nx + '|' + V.ms + '|' + T;
  if (softPts && softKey === key) return softPts;
  const { nx, ny, nz, rgba } = V, n = nx * ny * nz, cls = new Uint8Array(n);
  for (let i = 0, o = 0; i < n; i++, o += 4) {
    let c = 0;
    if (rgba[o] >= T) { c |= 1; if (rgba[o + 1] >= 128 || rgba[o + 3] >= 128) c |= 2; if (rgba[o + 3] >= 128) c |= 8; }
    if (rgba[o + 2] >= 128) c |= 4;
    cls[i] = c;
  }
  const sy = nx, sz = nx * ny, out = [];
  let cap = 1 << 18, cnt = 0;
  let P = new Float32Array(cap * 3), N = new Int8Array(cap * 3), K = new Uint8Array(cap);
  const grow = () => { cap *= 2; const P2 = new Float32Array(cap * 3); P2.set(P); P = P2; const N2 = new Int8Array(cap * 3); N2.set(N); N = N2; const K2 = new Uint8Array(cap); K2.set(K); K = K2; };
  const layers = [[4, 2], [1, 0], [2, 1]];   // [bit, channel]: gum surface, hard surface, tooth surface inside bone
  for (let z = 1; z < nz - 1; z++) for (let y = 1; y < ny - 1; y++) for (let x = 1; x < nx - 1; x++) {
    const i = x + y * sy + z * sz, c = cls[i]; if (!c) continue;
    for (let li = 0; li < 3; li++) {
      const b = layers[li][0]; if (!(c & b)) continue;
      if ((cls[i - 1] & b) && (cls[i + 1] & b) && (cls[i - sy] & b) && (cls[i + sy] & b) && (cls[i - sz] & b) && (cls[i + sz] & b)) continue;
      if (li === 2 && !(c & 1)) continue;                 // tooth surface only matters inside hard tissue
      const ch = layers[li][1], o = i * 4 + ch;
      let gx = rgba[o + 4] - rgba[o - 4], gy = rgba[o + sy * 4] - rgba[o - sy * 4], gz = rgba[o + sz * 4] - rgba[o - sz * 4];
      const glen = Math.hypot(gx, gy, gz) || 1;
      if (cnt >= cap) grow();
      P[cnt * 3] = V.bmin[0] + (x + 0.5) * V.vox; P[cnt * 3 + 1] = V.bmin[1] + (y + 0.5) * V.vox; P[cnt * 3 + 2] = V.bmin[2] + (z + 0.5) * V.vox;
      N[cnt * 3] = Math.round(-gx / glen * 127); N[cnt * 3 + 1] = Math.round(-gy / glen * 127); N[cnt * 3 + 2] = Math.round(-gz / glen * 127);
      // kind: 0 gum, 1 bone, 2 crown, 3 metal, 4 root (inside bone)
      K[cnt] = li === 0 ? 0 : li === 2 ? ((c & 8) ? 3 : 4) : ((c & 8) ? 3 : (c & 2) ? 2 : 1);
      cnt++;
    }
  }
  softKey = key;
  softPts = { P: P.subarray(0, cnt * 3), N: N.subarray(0, cnt * 3), K: K.subarray(0, cnt), n: cnt };
  return softPts;
}
...
function renderSoft3D() {
  ...
  for (let y = 0; y < h; y++) {                          // background gradient, matches the GPU renderer
    const t = y / h, r = (27 - 18 * t) | 0, g = (33 - 22 * t) | 0, b = (38 - 24 * t) | 0, col = (255 << 24) | (b << 16) | (g << 8) | r;
    D32.fill(col, y * w, y * w + w);
  }
  Z.fill(Infinity);
  const showGum = R.gums && R.mode !== 'xray', boneMode = R.boneA >= 0.5 ? 'solid' : R.boneA > 0.03 ? 'ghost' : 'hidden';
  const key = M.key, eye = M.eye, cp = clipPlane(M);
  const f = h / (2 * Math.tan(FOV / 2)), vox = V.vox;
  const { P, N, K, n } = pts;
  const lut = [];                                        // shade lookup per kind for speed
  for (let k = 0; k < 5; k++) lut.push(SOFT_COL[k]);
  for (let i = 0; i < n; i++) {
    const kind = K[i];
    if (kind === 0 && !showGum) continue;
    if (kind === 4 && boneMode === 'solid') continue;    // roots are hidden inside opaque bone anyway
    if (kind === 1 && boneMode === 'hidden') continue;
    const x = P[i * 3], y = P[i * 3 + 1], z = P[i * 3 + 2];
    if (cp && cp.n[0] * x + cp.n[1] * y + cp.n[2] * z > cp.d) continue;
    const cw = m[3] * x + m[7] * y + m[11] * z + m[15]; if (cw <= 1) continue;
    ...
    const ghost = kind === 1 && boneMode === 'ghost';
    let nx = N[i * 3] / 127, ny = N[i * 3 + 1] / 127, nz = N[i * 3 + 2] / 127;
    const vx = eye[0] - x, vy = eye[1] - y, vz = eye[2] - z, vl = Math.hypot(vx, vy, vz);
    if (nx * vx + ny * vy + nz * vz < 0) { nx = -nx; ny = -ny; nz = -nz; }
    const ndl = Math.max(0, nx * key[0] + ny * key[1] + nz * key[2]);
    const hemi = 0.5 + 0.5 * nz;
    let sh = kind === 2 ? 0.46 + 0.14 * hemi + 0.55 * ndl : 0.28 + 0.18 * hemi + 0.72 * ndl;
    let spec = 0;
    if (kind === 2 || kind === 3 || kind === 0) {
      const hx = key[0] + vx / vl, hy = key[1] + vy / vl, hz = key[2] + vz / vl, hl = Math.hypot(hx, hy, hz) || 1;
      spec = Math.pow(Math.max(0, (nx * hx + ny * hy + nz * hz) / hl), kind === 3 ? 60 : 40) * (kind === 3 ? 160 : kind === 2 ? 110 : 60);
    }
    const fog = 1 - 0.25 * Math.min(1, Math.max(0, (cw - (M.dist - M.diag / 2)) / M.diag));
    const col = lut[kind];
    let r = col[0] * sh * fog + spec, g = col[1] * sh * fog + spec, b = col[2] * sh * fog + spec;
    if (ghost) { r *= 0.55; g *= 0.55; b *= 0.6; }
    const packed = (255 << 24) | ((b > 255 ? 255 : b) << 16) | ((g > 255 ? 255 : g) << 8) | (r > 255 ? 255 : r);
    const s = Math.max(1, Math.min(5, Math.ceil(vox * f / cw * 1.3))), x0 = (sx - s / 2) | 0, y0 = (sy - s / 2) | 0;
    for (let yy = y0; yy < y0 + s; yy++) {
      if (yy < 0 || yy >= h) continue;
      for (let xx = x0; xx < x0 + s; xx++) {
        if (xx < 0 || xx >= w) continue;
        if (ghost && ((xx + yy) & 3)) continue;           // screen-door transparency for see-through bone
        const k = yy * w + xx;
        if (cw < Z[k]) { Z[k] = cw; D32[k] = packed; }
      }
    }
  }
